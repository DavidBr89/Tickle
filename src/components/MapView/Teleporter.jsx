 ...
 */
const teleportable = Wrapped =>
  class Teleportable extends React.Component {
    static propTypes = {
      telekey: PT.string
    };
    ...
    _render() {
      ...
      const container = teleregistry.get(this.props.telekey).node;
      ...
      ReactDOM.render(<Wrapped {...this.props} />, container);
    }
 ...
 */
const teleparent = Wrapped =>
  class Teleparent extends React.Component {
    constructor(props) {
      super(props);
      this.keys = [];
      this.namedKeys = {};
...
const teleregistry = {
  get(id) {
    if (!_reg[id]) {
      if (_reg[id] === null) {
        throw new Error(
          'Using a stale id! The contents have been garbage collected'
        );
      }
      _reg[id] = {
