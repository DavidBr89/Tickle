...
export const MediaField = ({
  values, onClick, edit, className
}) => <div className={className}>
  <div className="flex justify-end items-end">
    <MediaIcons values={values} />
  </div>
      </div>;
...
export const TitleField = ({
  onClick, edit, children, className, placeholder, hidden
}) => {
  if (hidden) return null;
  const titleCont = children || <span className="italic">{placeholder}</span>;
  return (
    <div className={`flex items-center items-center ${className}`} onClick={onClick}>
  ...
  return (
    ...
    >
      <div className="flex items-center text-xl">
        <div className="mr-1">{children || <span style={{ fontStyle: 'italic' }}>{placeholder}</span>}</div>
      </div>
    </div>
  );
};
