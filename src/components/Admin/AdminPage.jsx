...
function SelectCards(props) {
  const {className, cards, templateCard, urlConfig} = props;
  ...
  const cardsWithTemplate = [...cards, templateCard];

  const selectedCard =
    cardsWithTemplate.find(c => c.id === selectedCardId) || null;
  ...
  return (
    <details className={className} panelOpen={panelOpen}>
      ...
      <BlackModal visible={extended}>
        {extended && /* TODO */ <MetaCard {...selectedCard} />}
      </BlackModal>
    </details>
  );
}
